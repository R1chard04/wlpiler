...
MIPS::MIPS(std::istream &in){
    ...
    // std::ofstream f("statesAndTransitions.txt");
    ...
    //         f << "From " + currentState + " on input '" + inputSymbol + "' to " + nextState + "\n";
        ...
        if (nextState != "NF") {
            ...
            if (this->accepting[currState]) {
                if (currState == "NUM") {
                    ...
                    std::cout << "NUM" << " " << tmpToken << std::endl;
                }
                else if (std::find(this->idIntermediateStates.begin(), this->idIntermediateStates.end(), currState) != this->idIntermediateStates.end()) {
                    std::cout << "ID" << " " << tmpToken << std::endl;
                }
...
bool inIntRange(std::string strNum) {
    try {
        if ((unsigned long int)stoul(strNum) > 2147483647) { return false; }
    } catch (...) { return false; }
    return true;
}
